    ...
    for second_num in second_numbers:
        for third_letter in third_letters:
            for noise_level in noise_levels:
                ...
                output_filename = f"estim2/estim2_{first_num}_{second_num}_{third_letter}_{noise_level}.npy"

                # Run the particle filter for the current combination
                os.system(f"python particle_filter_kidnapped.py --map {landmark_maps[first_num]} --sensing {reading_filename} --num_particles {noise_level} --estimates {output_filename}")
